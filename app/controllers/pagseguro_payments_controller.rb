class PagseguroPaymentsController < Spree::BaseController
  ...
  def successful 
    ...
    unless @order.pagseguro_payment
      ...
      pagseguro_payment = PagseguroPayment.create(:order => @order, :email => params[:payer_email], :payer_id => params[:payer_id])
      @order.pagseguro_payment = pagseguro_payment
      pagseguro_payment.txns.create(:amount => params[:mc_gross].to_d, 
                                 :status => "Processed",
